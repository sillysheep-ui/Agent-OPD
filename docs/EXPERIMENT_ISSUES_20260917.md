...
- **E20｜评测结果写进了容器的临时层，宿主上根本不存在。** 第 18 次运行的评测链在第一次
  跑 140 局时暴露：`evaluate()` 把**宿主路径** `$EVALDIR/...` 直接当容器内的 `--output`
  传进去，容器里 `mkdir -p` 会成功并把 JSON 写在临时层，容器退出即丢；宿主侧
  `summarize()` 只能看到 missing。3 局冒烟因此"通过"却没有留下任何文件，140 局也是
  白跑（发现时只浪费了约 3 分钟，没有跑到终点）。
  **修复**：新增 `EVALDIR_CONTAINER=/runs/$(basename "$EVALDIR")`，所有交给容器的路径一律
  写成容器视角；并在每次评测结束后**在宿主侧断言输出文件存在**，不存在立刻失败退出，
  冒烟与主评测都套用这条断言。
  **归类**：这已经是同一类"视角混淆/事后才发现"的第三次（E17 落盘计划、E18 DTensor
  转换、E20 输出路径）。规则升级为两条：(1) 任何传给容器的路径必须显式以容器视角书写，
  并在起容器前对宿主文件做存在性校验；(2) 每段产物都要紧跟一条宿主侧的存在性断言，
  不允许"跑完再看"。

- **E21｜第 18 次运行完成并产出首个完整结果：传统逐 Token OPD 未超过冻结基线。**
  训练八卡 86 步零异常，最终 checkpoint 与完成清单落盘；评测链按同一服务、同一批
  140 局配对评测。结果：OPD **49/140 = 35.00%**，同批复测的冻结基线
  **51/140 = 36.43%**；逐局配对 45 两者都赢、4 仅 OPD 赢、6 仅 base 赢、85 都输，
  McNemar 精确检验 **p = 0.754**。详细表格与训练强度诊断见
  `docs/RESULTS_standard_opd_20260923.md`。
  **同时暴露一个方法论问题**：同一套提示词/协议下，冻结基线从 2026-09-21 的 45/140
  复测到 51/140（+6 局、+4.3 点）。这说明 140 局规模下的复测波动与我们要检测的效应同量级，
...
- **E27｜修正实现后传统 OPD 首次显著超过基线（2026-09-24）。** 运行
  `opd_native_20260924_01`（veRL 原生 PG OPD + 教师按自身模板渲染 + LoRA LR 3e-5、86 步）：
  `‖ΔW‖/‖W‖ = 1.23e-3`；同批 140 局配对 **66/140 = 47.14% vs 50/140 = 35.71%**，
  净 **+16 局**，McNemar **p = 0.0328**。提升集中在历史最弱的
  `pick_clean_then_place_in_recep`（2→12）。对照旧实现（k2 + 学生模板渲染 + LR 1e-6，
  ΔW 7.8e-5）的 49/140、配对 −2 局、p=0.75：**三处改动（教师渲染、目标函数、学习率标定）
...
- **E32｜定位并修复反事实分支的 $P_S$ 硬编码；保真门通过；单步干预在 8 局上无收益（2026-09-24）。**
  逐条对拍后根因明确：`counterfactual.py: _run_branch()` 在重建续跑历史时**硬编码**
  `replace_system(full_history, STUDENT_SYSTEM_PROMPT)`，而本研究冻结的 $P_S$ 是采集池里的
  AgentBoard 提示词（6948 字符），两者不同 → 分支的续跑策略被悄悄换成了另一个提示词，
  保真自然失败（1/8 vs 4/8）。**修复**：把 system prompt 作为参数
  （`student_system_prompt`，默认仍是仓库常量，向后兼容），由调用方传入冻结的 $P_S$；
  预验证脚本显式传入。
  **保真门复测（8 局）**：`baseline_wins=4`、`replayed_wins=4`（逐位复现）→ 测量有效。
  **同批 treatment 结果**：`corrected_wins=3`，delta = **−12.5 点**；7 个可纠正状态里
  **6 个无效、1 个变差、0 个变好**；早期状态 5 个、正向 0 个。
  **解读必须区分两件事**：(a) 这里测的是**环境内单步干预**（执行教师动作后由同一学生续跑），
  不是我们的训练臂；(b) 8 局样本量下 CI 极宽，不足以定论，但**没有任何正向信号**这一点与
  审计里的 R4 一致：学生整体策略才是瓶颈时，纠正单步的杠杆很小。
  **下一步**：(1) 若要给"干预式"变体定论，扩到 40 局（判定不变：≥+10 点才算有余量）；
  (2) 更直接地预验证**训练式**方法，应做小规模 A2（$B=50$ 采样 + 1–2 epoch + 40 局配对评测）。

- **E33｜逐步验收器上线；S1–S5 合同全过，但 A2 训练把模型打坏（0/40 vs 21/40）（2026-09-24）。**
  新增 `scripts/verify_pipeline.py`：把六步各自的**合同**写成可复跑检查，缺产物的步骤报 PENDING 而非失败。
  对当前产物（`preval_a2_*`）实测：
  `S1 states PASS`（240 局/5524 回合、hash 唯一、无空 admissible）；
  `S2 selection PASS`（注册策略、50 games、inclusion_prob 0.033–0.25）；
  `S3 teacher PASS`（attempts=ledger=requests=50、哈希已记录、acceptance 46/50 三层一致）；
  `S4 rows PASS`（46 行/46 games、`|weight_sum−1|max=0e0`、长度一致、至少 1 个监督 token、监督段为动作后缀）；
  `S5 training PASS`（loss 0.9337→0.0000、grad_norm 30.5、`|dW|/|W|=2.141e-03` 在带内）；
  **`S6 eval`：`opd 0/40` vs `base4b 21/40`**，配对 both=0、opd_only=0、**base_only=21**、neither=19，
  **净 −21，McNemar p<1e-4**。
  **结论**：**合同全过的同时结果崩塌**，说明当前"训练步"不成立——最可能是
  (a) 46 行 × 2 epoch 的记忆化（loss 4e-6）+ LR 1e-4 使 adapter 把行为推离可用区；
  或 (b) **训练行的历史表示/模板与评测侧不一致**（评测脚本的 `user_turn_style`/`assistant_history`
  与建行时的默认值若不同，学到的格式无法迁移）。
  **下一步（按代价排序，先诊断再调参）**：
  1. **格式一致性核对**：把一条训练行的 `input_ids` 解码出来，与评测侧同一状态实际发送的 prompt 逐字对比；
  2. **零训练对照**：用未训练的 adapter（step 0）跑同一 40 局，确认等于 base（排除服务/适配器加载问题）；
  3. **局部指标**：建"留出教师动作一致率"（在留出的若干状态上，adapter 的 argmax 是否命中教师动作），
     把它作为 S5 的**本地 KPI**——只有本地 KPI 上升、才允许看 S6；
  4. 之后才是 LR/epoch/数据量的扫描，且每改一项都要求"该步 KPI 上升且 S6 不劣化"。
  **纪律**：每一步的验收必须包含**该步的本地指标**；否则会出现"合同全过、结果归零"的假通过（本例）。
