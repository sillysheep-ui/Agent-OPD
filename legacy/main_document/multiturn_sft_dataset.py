...
class MultiTurnSFTDataset(Dataset):
    ...
    def _read_files_and_process(self):
        ...
        # Optional per-sample loss weight (state-level normalization for N>1 OPD)
        self.state_weight_key = 'state_weight'
        if self.state_weight_key in self.dataframe.columns:
            self.state_weights = self.dataframe[self.state_weight_key].astype(float).tolist()
        else:
    ...
    def __getitem__(self, item):
        ...
        # Token-level loss weight for per-state normalization (optional)
        sw = self.state_weights[item] if self.state_weights is not None else None
        if sw is not None:
            L = int(loss_mask.sum().item())
            loss_weight = loss_mask.float() * (sw / max(L, 1))
        else:
            # 无 weight 列: 全 1（与原始无加权路径等价）——None 会导致 collate 崩
